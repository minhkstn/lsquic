...
#define   MINH_BUFFER_SIZE          5000   //15000 //20000 //10000 //5000
#define   MINH_REBUF_THRESHOLD_EXIT 3000    //10000 //15000 //6000  // 3000
#define   MINH_SD                   1000
...
minh_SVC_backfilling_ABR(){
    ...
    if (minh_rebuf && minh_cur_buf >= MINH_REBUF_THRESHOLD_EXIT)    // stop rebuffering
    {
        ...
        if (last_seg_in_buffer > MINH_REBUF_THRESHOLD_EXIT/MINH_SD && last_seg_in_buffer < hung_max_seg_id_consideration)
        {
            sum_stall_duration += (long double)(lsquic_time_now() - streaming_start_time) / 1000 -
                                last_stall_start_time; //ms
            printf("\t\tstall duration so far: %.3f\n", sum_stall_duration);
    ...
    else //normal
    {
        ...
        if (just_exit_startup_phase || just_exit_rebuffering){
            ...
            last_seg_in_buffer = minh_client_seg; //first_seg_in_buffer -2 + (int) (minh_cur_buf/MINH_SD); //1;
            first_seg_in_buffer = (int) (minh_client_seg - (minh_cur_buf/MINH_SD) + 1);
        }
        else
        {
            first_seg_in_buffer = (int) (lsquic_time_now() - first_play_time - sum_stall_duration*1000)/(1000*MINH_SD) + 1;
        ...
        if (last_seg_in_buffer < minh_client_seg)
        {
          printf("Re-calculate buffered segments from %d to %d \n", first_seg_in_buffer, last_seg_in_buffer);
          last_seg_in_buffer = minh_client_seg;
          first_seg_in_buffer = last_seg_in_buffer - (int) (minh_cur_buf/MINH_SD) + 1;
        }
    ...
    // next_path =  MINH_PATH_SET[minh_client_seg % 12][cur_layer_id-1];  
    // printf("\tAlready set next path: %s\n", next_path);      
    }
    printf("\t\tNext seg: %d. num_layers: %d. Layer id: %d\n", 
        minh_client_seg, segment[minh_client_seg].num_layers, cur_layer_id); 
...
minh_update_stats(lsquic_stream_t *stream, lsquic_stream_ctx_t *st_h, int m_segment_id, bool retrans_layer){
    ...
    printf("[%.1Lf] Update stats: Segment %d Layer_id (1-4) %d current buff %.0f ms throughput %.3f kbps in %.0Lf ms\n", 
                                segment[m_segment_id].layer[layer_id-1].end_download_time, m_segment_id, layer_id, minh_cur_buf, minh_throughput, m_dowload_time);
...
// MINH [retransmission] ADD-E            
    ...
    if (system("sudo cp /home/minh/HTTP3_src/LSQUIC/lsquic/test/http_client.c /home/minh/HTTP3_src/LSQUIC/lsquic/complex_4g.sh /home/minh/HTTP3_src/LSQUIC/lsquic/results/")) {printf("Cannot copied files\n");}
